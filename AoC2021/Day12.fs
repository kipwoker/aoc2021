...
let traverseChildren routes node small maxSmall find' =
    routes
    |> Map.find node
    |> Array.map (fun destination -> find' routes destination small maxSmall |> List.map (fun path -> { State = path.State; Nodes = [node] @ path.Nodes }))
    |> Array.fold (fun state l -> state @ l) []

let rec find (routes: Map<string, string[]>) (node: string) (small: Set<string>) (maxSmall: int) : Path list =
    match node with
    | "start" -> traverseChildren routes node small maxSmall find
    | "end" -> [{ State = Valid; Nodes = ["end"] }]
    | n when Char.IsLower(n.[0]) ->
        let containsSmall = small.Contains(n)
        match (containsSmall, maxSmall) with
        | (true, 0) -> [{ State = Invalid; Nodes = [n] }]
...
let solve (input: string[]) (maxSmall: int) : string =
    ...
    let count = List.fold (fun state el -> state + (if el.State = Valid then 1 else 0)) 0
    let result = find routes "start" Set.empty maxSmall |> count
    result.ToString()
